...
import 'package:school_master_erp/views/screens/fees_screen/fees_structure_modal.dart';
...
class StactureScreen extends StatefulWidget {
  const StactureScreen({Key? key}) : super(key: key);

  @override
  State<StactureScreen> createState() => _StactureScreenState();
}

class _StactureScreenState extends State<StactureScreen> {
...
class ExampleSource extends AdvancedDataTableSource<FeesStucture> {
    ...
    final dialog = AwesomeDialog(
      ...
      btnOkOnPress: () {
        final d = AwesomeDialog(
          ...
          btnOkText: 'OK',
          btnOkOnPress: () {
            apiService.deletefeeStacture(id).whenComplete(() => setNextView());
          },
        );
        d.show();
      },
  ...
  @override
  Future<RemoteDataSourceDetails<FeesStucture>> getNextPage(
    ...
    if (response.statusCode == 200) {
      ...
      return RemoteDataSourceDetails(
        ...
        (data as List<dynamic>)
            .map(
              (json) => FeesStucture.fromJson(json as Map<String, dynamic>),
            )
            .toList(),
